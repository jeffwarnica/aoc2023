...
forward_re_bits = []
@s2d.each{|k,v| forward_re_bits << k }
forward_restr = '\d|' + forward_re_bits.join("|")
forward_re = Regexp.new(forward_restr)

puts "forward_re: [#{forward_restr}]"

reverse_re_bits = []
@s2d.each{|k,v| reverse_re_bits << k.reverse }
reverse_restr = '\d|' + reverse_re_bits.join("|")
reverse_re = Regexp.new(reverse_restr)

puts "reverse_re: [#{reverse_restr}]"
...
DATA.each_line do |line|
  ...
  first = found2int(fline.scan(forward_re).first)
  last = found2int(rline.scan(reverse_re).first.reverse)

  val = (first.to_i*10)+last.to_i
  puts "fline: #{fline}\nrline: #{rline}\n\t first : [#{first}] / [#{last}] == [#{val}]\n"
  total += val
end
puts "total: #{total}"
