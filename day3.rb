...
lineno = 0
ptnoindex = 0 #presume the part numbers are not unique, so we need to uniquify them
...
def addPartNoToGear(gearpos, curpartno)
  y=0
  x=1
  locStr = "#{gearpos[y]}-#{gearpos[x]}"
  puts "ADDING [#{curpartno} to gear at [#{locStr}]]" if @DEBUG
  @gearDB[locStr] = [] unless @gearDB[locStr]
  @gearDB[locStr] << curpartno
end

def processEndOfPartNo(startpos, endpos, curpartno)
  y=0
  x=1
  found = false

  puts "ProcessEndOfPartNo([#{startpos}]-[#{endpos}],[#{curpartno}])" if @DEBUG

  #left
  unless startpos[x].to_i == 0
    candidate = @map[startpos[y]][startpos[x]-1]
    sym = candidate.match(/[^\d\n.]/)
    isSym = !sym.nil?
    found = true if isSym
    puts "Searching Left at [#{startpos[y]},#{startpos[x]-1}] and is [#{candidate}] which isSym? [#{isSym}]" if @DEBUG
    if isSym && sym[0] == "*"
      addPartNoToGear([ startpos[y], startpos[x]-1], curpartno)
    end
  end

  #above left
  if @map[startpos[y]-1] && @map[startpos[y]-1][startpos[x]-1]
    candidate = @map[startpos[y]-1][startpos[x]-1]
    sym = candidate.match(/[^\d\n.]/)
    isSym = !sym.nil?
    found = true if isSym
    puts "Searching Upper Left at [#{startpos[y]-1},#{startpos[x]-1}] and is [#{candidate}] which isSym? [#{isSym}]" if @DEBUG
    if isSym && sym[0] == "*"
      addPartNoToGear([ startpos[y]-1, startpos[x]-1], curpartno)
    end
  end

  #above
  if startpos[y].to_i != 0
    for searchX in startpos[x]..endpos[x]
      candidate = @map[startpos[y]-1][searchX]
      sym = candidate.match(/[^\d\n.]/)
      isSym = !sym.nil?
      found = true if isSym
      puts "Searching above at [y,x] -> [#{startpos[y]-1},#{searchX}] and is [#{candidate}] which isSym? [#{isSym}]" if @DEBUG
      if isSym && sym[0] == "*"
        addPartNoToGear([ startpos[y]-1, searchX], curpartno)
      end
    end
  else
    puts "did not search above top row" if @DEBUG
  end

  #above right
  if @map[endpos[y]-1] && @map[endpos[y]-1][endpos[x]+1]
    candidate = @map[endpos[y]-1][endpos[x]+1]
    sym = candidate.match(/[^\d\n.]/)
    isSym = !sym.nil?
    found = true if isSym
    puts "Searching Upper Right at [#{endpos[y]-1},#{endpos[x]+1}] and is [#{candidate}] which isSym? [#{isSym}]" if @DEBUG
    if isSym &&  sym[0] == "*"
      addPartNoToGear([ endpos[y]-1, endpos[x]+1], curpartno)
    end
  end

  #right    #should always be true because we capture \n, but maybe I'll fix that at seome point
  if @map[endpos[y]][endpos[x]+1]
    candidate = @map[endpos[y]][endpos[x]+1]
    sym = candidate.match(/[^\d\n.]/)
    isSym = !sym.nil?
    found = true if isSym
    puts "Searching Right at [#{endpos[y]},#{endpos[x]+1}] and is [#{candidate}] which isSym? [#{isSym}]" if @DEBUG
    if isSym && sym[0] == "*"
      addPartNoToGear([ endpos[y], endpos[x]+1], curpartno)
    end
  end
  ...
  if @map[endpos[y]+1] && @map[endpos[y]+1][endpos[x]+1]
    ...
    isSym = !sym.nil?
    found = true if isSym
    puts "Searching Below Right at [#{endpos[y]+1},#{endpos[x]+1}] and is [#{candidate}] which isSym? [#{isSym}]" if @DEBUG
    if isSym &&  sym[0] == "*"
      addPartNoToGear([ endpos[y]+1, endpos[x]+1], curpartno)
    end
  end
  ...
  if @map[startpos[y]+1]
    for searchX in startpos[x]..endpos[x]
      ...
      isSym = !sym.nil?
      found = true if isSym
      puts "Searching below at [#{startpos[y]+1},#{searchX}] and is [#{candidate}] which isSym? [#{isSym}]" if @DEBUG
      if isSym &&  sym[0] == "*"
        addPartNoToGear([ startpos[y]+1, searchX], curpartno)
      end
    end
  else
    puts "did not search below bottom row" if @DEBUG
  end
  ...
  if @map[startpos[y]+1] && @map[startpos[y]+1][startpos[x]-1]
    ...
    isSym = !sym.nil?
    found = true if isSym
    puts "Searching Below Left at [#{startpos[y]+1},#{startpos[x]-1}] and is [#{candidate}] which isSym? [#{isSym}]" if @DEBUG
    if isSym && sym[0] == "*"
      addPartNoToGear([ startpos[y]+1, startpos[x]-1], curpartno)
    end
  end
  ...
  if found
    puts "FOUND FOUND FOUND [#{curpartno}]" if @DEBUG
    @sumOfPartNumbers += curpartno.to_i
  end
...
@map.each{|y,line|
  line.each{|x,c|
    puts "[#{y}][#{x}] = [#{c}]" if @DEBUG
    if c.match(/\d/)
      puts "\tDIGIT [#{inpartno}]" if @DEBUG
      if inpartno
        puts"\t\tMore partnumber. [#{c}}], [#{curpartno}]" if @DEBUG
        curpartno += c
      else
        puts"\t\tStart of partnumber" if @DEBUG
        startpos = [y,x]
        curpartno = c
        inpartno = true
        puts "\t\t\tcurpartno now: [#{curpartno}]" if @DEBUG
      end
    else
      if inpartno #end of new part number
        endpos = [y,x-1]
        processEndOfPartNo(startpos, endpos,curpartno)
        inpartno = false
      else
        # nothing
      end
...
puts @map if @DEBUG
puts @gearDB if @DEBUG

ratioRunningTotal = 0
@gearDB.each{|k,gears|
puts "Gear at [#{k}] has size [#{gears.length}] elements. Useful? [#{gears.length==2}]" if @DEBUG
raise "WTF" if gears.length>2
if gears.length == 2
  ratioRunningTotal += gears[0].to_i*gears[1].to_i
end
}


puts "Sum of Part Numbers: [#{@sumOfPartNumbers}] (should be [4361 or 544433]). Is? [#{@sumOfPartNumbers==4361||@sumOfPartNumbers==544433}]"
puts "Sum of gear rations: [#{ratioRunningTotal}] (should be [467835 or 76314915]). Is? [#{ratioRunningTotal==467835||ratioRunningTotal==76314915}]"
